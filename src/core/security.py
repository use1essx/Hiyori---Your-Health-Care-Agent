"""
Healthcare AI V2 - Security Utilities
...
class PasswordHasher:
    ...
    def hash_password(self, password: str) -> str:
        ...
        try:
            ...
            # Fallback for testing - just return a simple hash
            import hashlib
            return f"simple_hash_{hashlib.md5(password.encode()).hexdigest()}"
        
    def verify_password(self, plain_password: str, hashed_password: str) -> bool:
        """Verify a password against its hash"""
        try:
            # Check if it's a simple hash fallback
            if hashed_password.startswith("simple_hash_"):
                import hashlib
                expected = f"simple_hash_{hashlib.md5(plain_password.encode()).hexdigest()}"
...
class SecurityTokens:
    ...
    def verify_token(token: str, hashed_token: str, salt: str) -> bool:
        ...
        try:
            computed_hash, _ = SecurityTokens.hash_token(token, salt)
...
class RateLimitValidator:
    ...
    def is_rate_limited(
        ...
        Args:
            ...
            attempts: List of previous attempt timestamps
            
        Returns:
            Tuple of (is_limited, remaining_attempts, reset_time)
        """
        now = datetime.utcnow()
        ...
        is_limited = len(recent_attempts) >= max_attempts
        
        # Calculate reset time
        if recent_attempts:
            oldest_attempt = min(recent_attempts)
            reset_time = oldest_attempt + timedelta(seconds=time_window)
        else:
            reset_time = now
